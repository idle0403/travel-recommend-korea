...
|---------|---------|------------|----------|
| 🏛️ 관광지 (첫) | 5.0km | 3km 우선, 5km 초과 제외 | ✅ 일괄 처리 |
| 🍽️ 맛집 | 3.0km | 3km 우선, 5km 초과 제외 | ✅ 일괄 처리 |
| ☕ 카페 | 2.0km | 3km 우선, 5km 초과 제외 | ✅ 일괄 처리 |
| 🏛️ 관광지 (오후) | 4.0km | 3km 우선, 5km 초과 제외 | ✅ 일괄 처리 |
| 🌃 야경 | 4.0km | 3km 우선, 5km 초과 제외 | ✅ 일괄 처리 |
...
**개선된 설정**:
| 장소 유형 | 이전 반경 | 개선 반경 | 변경 이유 |
|----------|----------|----------|----------|
| 🏛️ **관광지** | 5.0km → 7.0km | **+40%** | 유명 관광지는 멀어도 방문 가치 있음 |
| 🍽️ **맛집** | 2.0km → 5.0km | **+150%** | 지방 도시 맛집은 4-6km 떨어질 수 있음 |
| ☕ **카페** | 1.0km → 3.0km | **+200%** | 분위기 좋은 카페 발굴 |
| 🌃 **야경/바** | 3.0km → 5.0km | **+67%** | 명소는 좀 멀어도 OK |

**효과**:
```javascript
// ai_schedule_framer.py (281-331번 줄)
"search_radius_km": 7.0,  // 관광지
"search_radius_km": 5.0,  // 맛집
"search_radius_km": 3.0,  // 카페
```

**결과**:
- ✅ 순천 무공해옹기곱창 (4.03km) → 이제 포함됨!
- ✅ 순천 시골집식당 (6.26km) → 이제 포함됨!
- ✅ 더 다양하고 질 높은 장소 추천 가능
...
유형 연속금지. 반경 넉넉하게 (관광7km, 맛집5km, 카페3km).
...
**간결하게** JSON만 출력 (코드블록X, 설명X):
{
  "schedule_frame": [
    {"day":1,"time_slot":"09:00-11:00","place_type":"tourist_attraction",
     "purpose":"오전 관광","search_keywords":["관광지","명소"],
     "search_radius_km":7.0,"priority":"high","expected_duration_minutes":120},
    {"day":1,"time_slot":"11:00-13:00","place_type":"restaurant",
     "purpose":"점심","search_keywords":["맛집"],
     "search_radius_km":5.0,"priority":"high","expected_duration_minutes":90}
  ]
}
